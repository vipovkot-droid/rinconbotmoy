...
def main_menu(is_admin: bool = False, lang: str = 'ru') -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(text="Мои реквизиты" if lang == 'ru' else "My requisites", callback_data="my_requisites", icon_custom_emoji_id="5902056028513505203"),
            InlineKeyboardButton(text="Создать сделку" if lang == 'ru' else "Create deal", callback_data="create_deal", icon_custom_emoji_id="5395732581780040886")
        ],
        [
            InlineKeyboardButton(text="Баланс" if lang == 'ru' else "Balance", callback_data="balance", icon_custom_emoji_id="5409048419211682843"),
            InlineKeyboardButton(text="Мои сделки" if lang == 'ru' else "My deals", callback_data="my_deals", icon_custom_emoji_id="5397782960512444700")
        ],
        [
            InlineKeyboardButton(text="Рефералы" if lang == 'ru' else "Referrals", callback_data="referrals", icon_custom_emoji_id="5325559344513691205"),
            InlineKeyboardButton(text="Язык / Lang" if lang == 'ru' else "Language / Lang", callback_data="change_lang", icon_custom_emoji_id="5902432207519093015")
        ],
        [
            InlineKeyboardButton(text="Техподдержка" if lang == 'ru' else "Support", callback_data="support", icon_custom_emoji_id="5238025132177369293")
        ]
    ]
    if is_admin:
        buttons.append([InlineKeyboardButton(text="Админ панель" if lang == 'ru' else "Admin panel", callback_data="admin_panel", icon_custom_emoji_id="⚙️")])
...
def requisites_menu(lang: str = 'ru') -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="TON-кошелёк", callback_data="edit_ton", icon_custom_emoji_id="5427168083074628963")],
        [InlineKeyboardButton(text="@username (Stars)", callback_data="edit_stars", icon_custom_emoji_id="5897692655273383739")],
        [InlineKeyboardButton(text="USDT-кошелёк", callback_data="edit_usdt", icon_custom_emoji_id="5287231198098117669")],
        [InlineKeyboardButton(text="BTC-кошелёк", callback_data="edit_btc", icon_custom_emoji_id="5829938927703691622")],
        [InlineKeyboardButton(text="Карта", callback_data="edit_card", icon_custom_emoji_id="5445353829304387411")],
...
def admin_panel(lang: str = 'ru') -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👑 Список админов" if lang == 'ru' else "👑 Admins list", callback_data="admin_list"),
         InlineKeyboardButton(text="👑 Удалить админа" if lang == 'ru' else "👑 Remove admin", callback_data="admin_remove_admin"),
...
def currency_menu(lang: str = 'ru') -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="⭐ STARS", callback_data="curr_STARS")   # ДОБАВЛЕНО
        ],
        [
...
def role_menu(lang: str = 'ru') -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=get_text('seller_btn', lang), callback_data="role_seller", icon_custom_emoji_id="5217822164362739968"),
            InlineKeyboardButton(text=get_text('buyer_btn', lang), callback_data="role_buyer", icon_custom_emoji_id="5312361253610475399")
        ],
        [
            InlineKeyboardButton(text="Назад в меню" if lang == 'ru' else "Back to menu", callback_data="main_menu", icon_custom_emoji_id="6039420807900303010")
        ]
    ])

def delivery_menu(deal_id: str, lang: str = 'ru') -> InlineKeyboardMarkup:
    text = "✅ Готово - ПЕРЕДАНО" if lang == 'ru' else "✅ DONE - TRANSFERRED"
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=text, callback_data=f"delivered_{deal_id}")]])
...
def confirm_receipt_menu(deal_id: str, lang: str = 'ru') -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=text, callback_data=f"confirm_receipt_{deal_id}")]])