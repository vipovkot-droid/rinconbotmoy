...
async def show_balance(callback: CallbackQuery):
    ...
    lang = db.get_user_language(user_id)
    user = db.get_user(user_id)
    balance = user.get('balance_rub', 0)
    deals = user.get('completed_deals', 0)
    min_deals = MIN_DEALS_FOR_WITHDRAW

    if balance == 0:
        balance_line = f"<tg-emoji emoji-id='5316583309541651465'>💔</tg-emoji> Вы баланс пока пуст"
    else:
        balance_line = f"<tg-emoji emoji-id='5893473283696759404'>💰</tg-emoji> <b>Ваш баланс:</b> {balance} RUB"
...
async def my_deals(callback: CallbackQuery):
    ...
    text = (
        f"<tg-emoji emoji-id='5197269100878907942'>✍️</tg-emoji> <b>Мои сделки</b>\n\n"
        f"<tg-emoji emoji-id='5406745015365943482'>⬇️</tg-emoji> <b>Всего:</b> {total}\n"
        f"<tg-emoji emoji-id='5206607081334906820'>✔️</tg-emoji> <b>Завершено:</b> {completed}\n\n"
    )
    for deal in deals[:8]:
        if deal['status'] == 'completed':
            ...
            status_emoji = "<tg-emoji emoji-id='5451732530048802485'>⏳</tg-emoji>"
    ...
    back_button_emoji = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Назад в меню", callback_data="main_menu", icon_custom_emoji_id="6039420807900303010")]
    ])
    await callback.message.delete()
    await callback.message.answer(text, reply_markup=back_button_emoji, parse_mode="HTML")
    await callback.answer()
...
async def referrals(callback: CallbackQuery):
    ...
    text = (
        f"<tg-emoji emoji-id='5377660214096974712'>🛍</tg-emoji> <b>FRAGMENT DEALS</b> <tg-emoji emoji-id='5377660214096974712'>🛍</tg-emoji>\n\n"
        f"<tg-emoji emoji-id='5440660757194744323'>‼️</tg-emoji> <b>Реферальная программа</b>\n\n"
        f"<tg-emoji emoji-id='5271604874419647061'>🔗</tg-emoji> <b>Ваша ссылка:</b>\n<code>{ref_link}</code>\n\n"
        f"<tg-emoji emoji-id='5453957997418004470'>👥</tg-emoji> <b>Рефералов:</b> 0\n"
        f"<tg-emoji emoji-id='5375296873982604963'>💰</tg-emoji> <b>Заработано:</b> 0.0 TON\n\n"
        f"<b>Бонус: 50% от комиссии с каждой сделки реферала!</b> <tg-emoji emoji-id='5895440460322706085'>📌</tg-emoji>"
    )
    buttons = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Скопировать реф. ссылку", callback_data="copy_ref", icon_custom_emoji_id="5197269100878907942")],
        [InlineKeyboardButton(text="Назад в меню", callback_data="main_menu", icon_custom_emoji_id="6039420807900303010")]
    ])
    await callback.message.delete()
...
async def support(callback: CallbackQuery):
    ...
    lang = db.get_user_language(user_id)
    text = (
        f"<tg-emoji emoji-id='5395732581780040886'>🤝</tg-emoji> <b>FRAGMENT DEALS</b> <tg-emoji emoji-id='5395732581780040886'>🤝</tg-emoji>\n\n"
        f"<tg-emoji emoji-id='5893365724830765382'>🛡</tg-emoji> <b>Техподдержка</b>\n\n"
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    ...
                    icon_custom_emoji_id="5229192892710402006"   # 🏴󠁧󠁢󠁥󠁮󠁧󠁿
                )
            ],
            [
                InlineKeyboardButton(
...
async def select_currency(callback: CallbackQuery, state: FSMContext):
    ...
    if missing_field:
        ...
        text = (
            f"<tg-emoji emoji-id='5274099962655816924'>❗️</tg-emoji> <b>Для создания сделки в валюте {currency} необходимо указать {field_name}.</b>\n\n"
...
async def edit_ton(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "<tg-emoji emoji-id='5427168083074628963'>💎</tg-emoji> Введите новый TON-кошелёк:",
...
async def edit_card(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "<tg-emoji emoji-id='5445353829304387411'>💳</tg-emoji> Введите номер карты:",
        reply_markup=back_button(),
        parse_mode="HTML"
    )
...
async def edit_stars(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "<tg-emoji emoji-id='5924870095925942277'>⭐️</tg-emoji> Введите @username для Stars (без @):",
        reply_markup=back_button(),
        parse_mode="HTML"
    )
...
async def edit_usdt(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "<tg-emoji emoji-id='6039802097916974085'>🪙</tg-emoji> Введите USDT-адрес (TRC20):",
        reply_markup=back_button(),
        parse_mode="HTML"
    )
...
async def edit_btc(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "<tg-emoji emoji-id='5816788957614053645'>🪙</tg-emoji> Введите BTC-адрес:",
        reply_markup=back_button(),
        parse_mode="HTML"
    )
...
async def save_requisite(message: Message, state: FSMContext):
    ...
    if data.get('return_to_deal') and data.get('pending_deal'):
        ...
        if role and currency:
            ...
            emoji = "💳" if currency in ['RUB', 'UAH', 'KZT', 'BYN'] else "🪙" if currency == 'TON' else "💎" if currency == 'USDT' else "₿"
...
async def process_description(message: Message, state: FSMContext):
    ...
    creator_role = data['role']   # 'seller' или 'buyer' — ДОБАВЛЕНО
    
    db.create_deal(deal_id, user_id, data['currency'], data['amount'], message.text, creator_role)   # ИЗМЕНЕНО
...
async def join_deal_as_buyer(message: Message, deal_id: str):
    ...
    creator_role = deal.get('creator_role', 'seller')   # по умолчанию seller (для старых сделок)
    card_number = db.get_setting("card_number")

    creator = db.get_user(deal['seller_id'])   # создатель сделки
    joiner = db.get_user(user_id)              # присоединившийся

    # Определяем, кто будет покупателем, а кто продавцом
    if creator_role == 'seller':
        # Создатель – продавец, присоединившийся – покупатель
    ...
    # Получаем объекты продавца и покупателя
    seller = db.get_user(seller_id)
    buyer = db.get_user(buyer_id)

    # Текст для покупателя (короткий, с кнопкой оплаты)
    buyer_text = (
        f"<blockquote>"
    ...
    # Текст для продавца (подробный, без кнопки оплаты)
    seller_text = (
    ...
    # Ответ присоединившемуся в чате (тот, кто нажал ссылку)
...
async def pay_from_balance(callback: CallbackQuery):
    ...
    if buyer.get(balance_field, 0) < deal['amount']:
        await callback.answer(get_text('not_enough_balance', lang), show_alert=True)
        return

    db.update_balance(user_id, deal['amount'], deal['currency'], "sub", deal_id)
...
async def seller_delivered(callback: CallbackQuery):
    ...
    if deal['status'] != 'waiting_delivery':
        await callback.answer("❌ Сделка не в статусе ожидания доставки", show_alert=True)
        return
    ...
    try:
        ...
        print(f"Не удалось уведомить покупателя: {e}")
    ...
    seller_confirm_buttons = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Назад в меню", callback_data="main_menu", icon_custom_emoji_id="6039420807900303010")]
    ])
    await callback.message.edit_text(seller_confirm_text, reply_markup=seller_confirm_buttons, parse_mode="HTML")
    await callback.answer()
...
async def buyer_confirm_receipt(callback: CallbackQuery):
    ...
    if deal['status'] != 'waiting_delivery':
        await callback.answer("❌ Сделка не в статусе ожидания подтверждения", show_alert=True)
        return
    ...
    complete_text = (
        f"<tg-emoji emoji-id='5461151367559141950'>🎉</tg-emoji> <b>Сделка #{deal_id} успешно завершена!</b>\n\n"
        f"<blockquote>"
        f"<tg-emoji emoji-id='5337080053119336309'>👍</tg-emoji> <b>Спасибо за доверие!</b>"
        f"</blockquote>"
    )
    try:
...
async def process_withdraw_amount(message: Message, state: FSMContext):
    user_id = message.from_user.id
    lang = db.get_user_language(user_id)
    match = re.match(r"(\d+(?:\.\d+)?)\s*(RUB|TON|USDT|BTC)", message.text.upper())
    ...
    if not wallet:
        await message.answer(
            f"<tg-emoji emoji-id='5274099962655816924'>❗️</tg-emoji> У вас не указан кошелёк для вывода {currency}!\n\n"
            f"Пожалуйста, добавьте реквизиты в разделе «Мои реквизиты»",
            reply_markup=back_button(),
            parse_mode="HTML"
        )
    ...
    for admin_id in admins:
        try:
            await bot.send_message(
                ...
                f"<tg-emoji emoji-id='5893473283696759404'>💰</tg-emoji> Сумма: {amount} {currency}\n"
                f"<tg-emoji emoji-id='5238025132177369293'>🆘</tg-emoji> Кошелёк: <code>{wallet}</code>\n\n"
                f"<tg-emoji emoji-id='5451732530048802485'>⏳</tg-emoji> Обработайте в течение 24 часов.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_remove_admin_list(callback: CallbackQuery):
    ...
    if user.get('username') != ADMIN_USERNAME:
        await callback.answer("❌ Только главный администратор (@TonRincon) может удалять админов!", show_alert=True)
        return
    admins = db.get_all_admins()
    if not admins:
        await callback.message.delete()
